...
The results show a clear trend: the Histogram significantly outperforms the SIFT approach on both GTSRB and CIFAR-10. Bei GTSRB sehen wir ein klares Muster
Regarding hyperparameters, we observe the expected behavior: deeper trees ($d=15$) and a higher number of estimators ($n=90$) yield the best performance, indicating that the models benefit from increased complexity.
...
To better understand why the Color Histogram consistently outperformed the SIFT approach, we generated confusion matrices for the best performing Random Forest models from our GridSearch.

Figure \ref{fig:cm_cifar} shows the confusion matrices for the CIFAR-10 dataset.
