...
Since Support Vector Machines (SVMs) are well suited for high-dimensional feature spaces, we applied them to both the CIFAR-10 and GTSRB datasets. SVMs aim to find a decision boundary that maximizes the margin between classes. Similar to Logistic Regression, two hyperparameters are explored: the regularization parameter (C), which controls the penalty for misclassified samples, and the penalty parameter, which specifies the regularization norm.

SVMs generally achieve stronger performance than Logistic Regression on both datasets. However, they are computationally more expensive.
...
\caption{GTSRB: Training curves for best model (Pre=Yes, Frz=No, Aug=Yes, Drop=0.0)}
...
\caption{CIFAR-10: Training curves for best model (Pre=Yes, Frz=No, Aug=Yes, Drop=0.0)}
...
@nayma top 3 table for 1. ML algorithm

@nayma top 3 table for 2. ML algorithm
...
\hline
 \textbf{Dropout} & \textbf{Augment} & \textbf{Test Accuracy (\%) } & \textbf{Epochs} & \textbf{Total train time (s)} & \textbf{Total test time (s)} \\ \hline
