...
As our second model, we used \textbf{ResNet18}, a more modern CNN architecture compared to LeNet5. The key feature of ResNet is its \textbf{residual connections}. This feature addresses the vanishing gradient problem in deep networks. Basically, they allow the network to skip layers; instead of learning complete transformations, each layer learns only the residual difference. 

A major advantage of ResNet18 over LeNet5 is its capability for \textbf{transfer learning}. This allows us to use ImageNet pretrained weights as a starting point. We tested both training from scratch and using ImageNet pretrained weights. Additionally, we have the option to \textbf{freeze the backbone}. When activated, the convolutional layers are frozen and not updated during training (advantage: significant resource savings). In this configuration, the frozen backbone provides learned feature representations from ImageNet, while only the final classification layer is trained. This is why freezing doesn't make sense when we're not using transfer learning – freezing random weights provides no benefit. 
...
\begin{itemize}
    ...
    \item \textbf{Augmentation:} Yes, No
\end{itemize}

Other parameters were kept at default values:
\begin{itemize}
    \item \textbf{Image Size:} 32$\times$32
    \item \textbf{Optimizer:} Adam
    \item \textbf{Learning Rate:} 0.001
    \item \textbf{Activation Function:} ReLU (ResNet default)
\end{itemize}
